...
| `Quake-2-master/game/g_utils.c` | function | `vectoangles` | `packages/game/src/g_utils.ts` | `vectoangles` | Partiel | A redecouper | Port TS proprietaire compare et teste; commentaire mis a jour dans `packages/game/src/g_utils.ts`; correction test `scripts/verify/quake2-g-utils.ts`. Manque ouvert: `packages/game/src/g_weapon.ts` garde un helper local `vectoangles` au lieu de consommer l'export officiel. |
