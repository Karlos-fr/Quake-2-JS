...
- Matrice C/H: aucune ligne proprietaire pour ce fichier. Les portages `ref_gl` appeles restent proprietaires de leurs fichiers (`gl_rmain.ts`, `gl_draw.ts`, `gl_image.ts`, `gl_rmisc.ts`, `gl-model-loader.ts`).
