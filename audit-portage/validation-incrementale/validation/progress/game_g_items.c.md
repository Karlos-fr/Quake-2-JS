...
- `Weapon_Shotgun` reference par `g_items.c` et porte dans `packages/game/src/p_weapon.ts`, avec entree `weapon_shotgun` alignee dans `packages/game/src/g_items.ts`.
...
- `Weapon_SuperShotgun`, en gardant la meme attention au rattachement `g_items.c` declarations/table vers le corps original `game/p_weapon.c` et le port `packages/game/src/p_weapon.ts`.
