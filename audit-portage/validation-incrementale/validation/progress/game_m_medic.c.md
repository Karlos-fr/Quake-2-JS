...
## 2026-05-06 - Lot cable/resurrection et fermeture fichier

- Lot traite: `medic_hook_launch`, `medic_cable_offsets`, `medic_cable_attack`, variables locales `tr`/`distance`, `medic_hook_retract`, `medic_frames_attackCable`, `medic_move_attackCable`, entree declarative `medic_frames_attackCable`, `medic_attack`, `medic_checkattack`, `SP_monster_medic`.
- Verdict: fichier termine; `Valide` pour 11 lignes du lot, `Non applicable` pour les variables locales `tr` et `distance`.
- Ownership: `packages/game/src/m_medic.ts` reste le fichier proprietaire; les fonctions exportees sont aussi disponibles via `packages/game/src/index.ts`; `medic_move_attackCable` est restaure par le registre save. `medic_cable_offsets` et `medic_frames_attackCable` restent des tables proprietaires internes au module TS, comme les tables declaratives precedentes.
- Source C vs TS: cable compare sur sons hook launch/hit/heal/retract, offsets, garde enemy inuse, projection `G_ProjectSource`, distance max 256, pitch max 45, trace `MASK_SHOT`, reinitialisation du cadavre avant `ED_CallSpawn`, relance du `think`, flag `AI_RESURRECTING`, reprise d'ancien ennemi client via `FoundTarget`, ajustement start/end et emission `TE_MEDIC_CABLE_ATTACK`. `medic_attack`, `medic_checkattack` et `SP_monster_medic` compares aux branches C et aux callbacks/assets/bounds/model/health/start move.
- Commentaires d'en-tete: ajoutes pour `medic_hook_launch`, `medic_cable_attack`, `medic_hook_retract`, `medic_attack`, `medic_checkattack`, `SP_monster_medic`.
- Runtime: integre via `monster_medic` dans `g_spawn.ts`, `SP_monster_medic`, `monsterinfo.attack`/`checkattack`, `ai_checkattack`, `G_RunFrame`/`M_MoveFrame`; le cable produit sons, resurrection du monstre cible, temp entity visible et restauration save des callbacks/moves.
- `apps/web`: aucune logique medic parallele; le flux full-game/web consomme le runtime porte, les snapshots, les sons et les temp entities. Le cable visible est attendu dans le navigateur et passe par le paquet temp entity client puis le render source.
- `packages/renderer-three`: sorties visibles attendues = frames/modele du medic, monstre ressuscite, beam `TE_MEDIC_CABLE_ATTACK` utilisant le segment parasite, et effets/audio associes cote client. Le client parse `TE_MEDIC_CABLE_ATTACK` comme beam, `CL_AddTEntPacket` le place dans les beams, `apps/web` appelle `beamSync.apply`, et `renderer-three` consomme la refresh frame via `createThreeBeamSync`/`R_DrawBeam`.
- Corrections appliquees: commentaires de portage ajoutes dans `packages/game/src/m_medic.ts`; harness `scripts/verify/quake2-m-medic.ts` renforce pour sons hook, guards cable, spawn resurrection, save registry et temp entity; harness `scripts/verify/quake2-m-medic-source-parity.ts` renforce pour comparer `medic_cable_offsets` C vs TS.
- Tests lances OK: `npm run verify:m-medic`, `npm run verify:m-medic:source-parity`, `npm run verify:m-medic:header`, `npm run verify:beam-sync`, `npm run verify:full-game:render-source`, `npm run verify:full-game:three-renderer`, `npm run verify:web-render-order`, `npm run typecheck`, `git diff --check`.
- Prochain lot recommande: aucun; toutes les lignes de `game_m_medic.c.md` sont `Valide` ou `Non applicable`.
