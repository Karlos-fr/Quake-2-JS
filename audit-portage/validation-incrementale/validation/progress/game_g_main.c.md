...
- Corriger le lot bug-driven `SpawnEntities`/allocation edicts avant de reprendre `ReadLevel`: remplacer la preconstruction de toute la lump par une allocation sequentielle compatible C (`G_Spawn` + reutilisation des frees), puis ajouter une verification city1 garantissant `ge.num_edicts <= maxentities` et aucune entite snapshot visible au-dela de `MAX_EDICTS - 1`.
