...
- Cibles comparees: `packages/formats/src/qfiles.ts`, `packages/formats/src/index.ts`, `packages/qcommon/src/cmodel.ts`, `apps/web/src/main.ts`, `apps/web/src/full-game-local-session.ts`, `packages/renderer-three/src/gl-model-loader.ts`, `packages/renderer-three/src/gl-model.ts`, `packages/renderer-three/src/gl_light.ts`, `packages/renderer-three/src/gl_rsurf.ts`, `packages/renderer-three/src/gl-world-scene-adapter.ts`.
...
- renderer-three: attendu et verifie. Les sorties BSP visibles sont consommees pour surfaces, edges/surfedges, faces/lightmaps/styles, leaf visibility, PVS, scene/world, alpha/sky/warp et dynamic lightmaps via `gl-model-loader`, `gl_rsurf`, `gl_light` et le world adapter.
