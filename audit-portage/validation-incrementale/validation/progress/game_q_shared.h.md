...
- Traiter le bloc `STAT_*` de `STAT_HEALTH_ICON` a `MAX_STATS` si le lot reste coherent. Reprendre ensuite les `DF_*`/Rogue version/angle macros, puis une decision explicite sur les producteurs gameplay Rogue absents avant de fermer les `MZ2_*` partiels.
