...
- Runtime: `flymonster_start` est appele par les spawn routines volantes (`m_boss2`, `m_float`, `m_flyer`, `m_hover`, `m_insane`), pose `FL_FLY`, arme le think differe `flymonster_start_go`, puis passe par `monster_start`; `swimmonster_start_go` est atteint depuis `swimmonster_start`, pose les defaults `yaw_speed`/`viewheight`, appelle `monster_start_go` et conserve le masquage trigger-spawn via `monster_triggered_start`.
