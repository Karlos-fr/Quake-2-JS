...
- Tests de reference lances:
  ...
  - `npm run typecheck`
- Decisions:
  - `CL_EntityEvent` est porte sous le nom `CL_BuildEntityEventEffects` dans `packages/client/src/cl_fx.ts`; le renommage est justifie par l'adapter structure en `ClientActionEffect`, et l'en-tete indique `Original name: CL_EntityEvent`, `Source: client/cl_fx.c`, `Category: Ported`, `Fidelity level: Close`.
  - Le switch C est conserve: `EV_ITEM_RESPAWN` emet `items/respawn1.wav` sur `CHAN_WEAPON`/`ATTN_IDLE` et `CL_ItemRespawnParticles`; `EV_PLAYER_TELEPORT` emet `misc/tele1.wav` sur `CHAN_WEAPON`/`ATTN_IDLE` et `CL_TeleportParticles`; `EV_FOOTSTEP` respecte `cl_footsteps` et choisit `player/step[1-4].wav` sur `CHAN_BODY`/`ATTN_NORM`; les chutes emettent `player/land1.wav`, `*fall2.wav` et `*fall1.wav` sur `CHAN_AUTO`/`ATTN_NORM`; `EV_OTHER_TELEPORT` reste sans effet dans cette fonction comme le defaut C.
  - `CL_EntityEvent` est atteignable depuis le runtime client normal via `CL_ParseServerMessage` / `CL_ParseFrame` -> `CL_FireEntityEvents` -> `hooks.onEntityEvent` et via le builder `CL_BuildFrameEntityEventEffects`; `EV_OTHER_TELEPORT` reste traite dans le no-lerp de `CL_DeltaEntity`, pas comme effet sonore/particule.
  - `apps/web` doit consommer ce flux parce qu'il declenche des sons et, pour item respawn/player teleport, des particules visibles: `apps/web/src/full-game.ts` appelle `CL_BuildEntityEventEffects` dans `onEntityEvent`, route les sons vers `S_DMA_StartSound`, et applique `CL_ItemRespawnParticles` / `CL_TeleportParticles` sans logique parallele.
  - `renderer-three` doit consommer les sorties visibles produites par `EV_ITEM_RESPAWN` et `EV_PLAYER_TELEPORT`: elles passent par `CL_AddParticles` -> `ClientRefreshFrame.particles` -> `createThreeParticleSync` / `R_DrawParticles`; les sons de footstep/chute/teleport/respawn ne sont pas du ressort de `renderer-three`.
  ...
  - `apps/web` doit consommer ce flux parce qu'il produit des particules et une dlight visibles: le navigateur consomme `ClientRefreshFrame.particles` et `ClientRefreshFrame.lights` issus du runtime client, sans logique trap parallele detectee dans `apps/web`.
  - `renderer-three` doit consommer les sorties visibles de `CL_TrapParticles`: particules via `ClientRefreshFrame.particles` -> `createThreeParticleSync` / `R_DrawParticles`, et dlight via `ClientRefreshFrame.lights` -> `createThreeDlightSync`; preuves `verify:particle-sync`, `verify:web-render-order` et `verify:full-game:three-renderer`.
  - `CL_BFGExplosionParticles` conserve le flux C des 256 particules: couleur `0xd0+(rand&7)`, origine `org + rand%32 - 16`, vitesse `rand%384 - 192`, acceleration Z `-PARTICLE_GRAVITY`, alpha `1.0` et decay `-0.8/(0.5+frand()*0.3)`.
  - `CL_BFGExplosionParticles` est atteignable depuis le runtime client normal via `CL_ParseTEnt` / `CL_ParseServerMessage` pour `TE_BFG_BIGEXPLOSION`, puis `CL_ExecuteTempEntityEffects`.
  - `apps/web` doit consommer ce flux parce qu'il produit des particules visibles de temp entity BFG: `CL_BuildTempEntityEffects` expose `bfg-explosion-particles` et le flux full-game consomme ensuite `ClientRefreshFrame.particles`, sans logique particule BFG parallele detectee.
  - `renderer-three` doit consommer la sortie visible particules via `ClientRefreshFrame.particles` -> `createThreeParticleSync` / `R_DrawParticles`, couvert par `verify:particle-sync` et `verify:full-game:three-renderer`.
  ...
  - `CL_FlyEffect` conserve le flux C de ramp-up/ramp-down: initialise `fly_stoptime = cl.time + 60000` quand expire, calcule `starttime`, tronque le `count` via `Math.floor`, conserve le plateau `162`, puis delegue a `CL_FlyParticles`.
  - Les temporaires C `n`, `count` et `starttime` sont des artefacts locaux portes dans le corps TS; aucune entite TS publique separee n'est attendue.
  - `CL_BfgParticles` conserve le flux C: reutilisation de la table partagee `avelocities`, boucle sur `NUMVERTEXNORMALS`, `ltime = cl.time / 1000.0`, `BEAMLENGTH = 16`, origine `ent.origin + bytedirs[i] * sin(ltime+i)*64 + forward*16`, vitesse/acceleration nulles, couleur `floor(0xd0 + VectorLength(v)/90*7)`, alpha `1.0 - VectorLength(v)/90` et `alphavel = -100`.
  - Les temporaires C `i`, `angle`, `forward`, `dist`, `v` et `ltime` de `CL_BfgParticles` sont portes dans `spawnBfgParticles`; le second `BEAMLENGTH` est porte par `FLY_BEAM_LENGTH`.
  - `CL_BfgParticles` est maintenant atteignable depuis le runtime client normal via `CL_BuildRefreshFrame` -> `CL_BuildPacketEntitySnapshots` -> `CL_ExecutePacketEntityEffects` pour `EF_BFG | EF_ANIM_ALLFAST`; la dlight BFG associee conserve le rayon C `200` dans ce cas.
  - `apps/web` doit consommer ce flux parce qu'il produit des particules BFG et une dlight visibles: le navigateur consomme `ClientRefreshFrame.particles`/`lights` issus du runtime client, sans logique BFG parallele detectee dans `apps/web`.
  - `renderer-three` doit consommer les sorties visibles de `CL_BfgParticles`: particules via `ClientRefreshFrame.particles` -> `createThreeParticleSync` / `R_DrawParticles`, dlight via `ClientRefreshFrame.lights` -> `createThreeDlightSync`, et entite translucide EF_BFG via snapshots refresh.
...
- Blocages: aucun blocage restant pour `CL_EntityEvent`.
- Prochain lot recommande: aucun lot restant dans `client_cl_fx.c.md`; laisser le coordinateur recalculer/mettre a jour `AVANCEMENT_GLOBAL.md`.
