...
- Blocages: aucun blocage restant pour `CL_BubbleTrail`.
- Prochain lot recommande: `BEAMLENGTH` puis `CL_FlyParticles` en lot borne; verifier aussi les locaux `i`, `angle`, `forward`, `dist` et `ltime` si la matrice les rattache au lot; garder `CL_FlyEffect` pour une session separee si cela devient large.
