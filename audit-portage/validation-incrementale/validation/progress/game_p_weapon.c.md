...
## Session 2026-05-06 - blocs railgun et BFG

- Lot traite: `weapon_railgun_fire`, variables locales `start`/`offset`/`damage`/`kick`, `Weapon_Railgun`, tables locales `pause_frames` et `fire_frames`, puis `weapon_bfg_fire`, variables locales `damage`/`damage_radius` et doublon genere, `Weapon_BFG`, tables locales `pause_frames` et `fire_frames`.
- Comparaison C/TS railgun: degats solo/deathmatch `150`/`100`, kick `250`/`200`, quad applique aux degats et au kick, `AngleVectors`, kick origin `forward * -3`, kick angle `-3`, projection `[0, 7, viewheight - 8]`, `fire_rail`, muzzleflash `MZ_RAILGUN | is_silenced`, `PlayerNoise`, avance gunframe et consommation d'une slug sauf `DF_INFINITE_AMMO` compares contre le C. `Weapon_Railgun` appelle `Weapon_Generic(ent, 3, 18, 56, 61, {56, 0}, {4, 0}, weapon_railgun_fire)`; le frame idle-last `56` wrappe vers `19` comme dans `Weapon_Generic` C avant toute pause.
- Comparaison C/TS BFG: degats solo/deathmatch `500`/`200`, rayon `1000`, warmup frame `9` avec muzzleflash `MZ_BFG | is_silenced`, bruit a l'origine joueur et retour sans projectile, re-verification des `50` cells au frame `17`, quad, kick origin `forward * -2`, `v_dmg_pitch = -40`, `v_dmg_roll = crandom() * 8`, `v_dmg_time = level.time + DAMAGE_TIME`, projection `[8, 8, viewheight - 8]`, `fire_bfg` vitesse `400`, avance gunframe, `PlayerNoise` et consommation de `50` cells sauf `DF_INFINITE_AMMO` compares contre le C. `Weapon_BFG` appelle `Weapon_Generic(ent, 8, 32, 55, 58, {39, 45, 50, 55, 0}, {9, 17, 0}, weapon_bfg_fire)`.
- Commentaires d'en-tete: `weapon_railgun_fire`, `Weapon_Railgun`, `weapon_bfg_fire` et `Weapon_BFG` verifies avec `Original name`, `Source: game/p_weapon.c`, `Category: Ported`, niveau de fidelite et comportement.
- Runtime: integre via `g_items` (`weapon_railgun` -> `Weapon_Railgun`, `weapon_bfg` -> `Weapon_BFG`), `Think_Weapon`, `ClientThink`/`ClientBeginServerFrame`, `G_RunFrame`, `local-game-bootstrap` et hooks `fire_rail`/`fire_bfg`.
- apps/web: le navigateur declenche le tir via les flux serveur/local/full-game et consomme le runtime porte; le binding `BFG10K: "0"` est un adapter d'input, pas une logique parallele de tir.
- renderer-three: sorties visibles attendues = view/world models `v_rail`/`g_rail` et `v_bfg`/`g_bfg`, muzzleflash/dlights `MZ_RAILGUN`/`MZ_BFG`, rail trail `TE_RAILTRAIL`, projectile BFG `EF_BFG | EF_ANIM_ALLFAST`, sprites `s_bfg*`, sons, beams/lasers et explosions/temp entities BFG; elles sont consommees par snapshots, `local-gameplay-sync`, `cl_fx`, refresh entity/dlights et renderer-three.
- Corrections: ajout d'assertions ciblees dans `scripts/verify/quake2-p-weapon.ts` pour `weapon_railgun_fire` direct, offset/projection, degats/kick solo/deathmatch/quad, bit silencieux, ammo infini, tables railgun et frame non-fire; et pour BFG warmup direct, bit silencieux, projection complete, degats solo/deathmatch/quad, rayon, kick, abort cells, ammo infini, tables et frame non-fire.
- Tests lances: `npm run verify:p-weapon`, `npm run typecheck`, `npm run verify:local-gameplay-sync`, `npm run verify:cl-fx`, `npm run verify:full-game:three-renderer`, `npm run verify:refresh-entity:weapon`.

## Prochain lot recommande

Aucun lot restant dans `game_p_weapon.c.md`: toutes les lignes sont `Valide` ou `Non applicable`.
