...
- renderer-three: `Com_sprintf` est consomme par `packages/renderer-three/src/gl_model.ts` pour produire les chemins `maps/*.bsp` et `textures/*.wal`, donc les sorties visibles modeles/images restent branchees; les comparateurs `Q_str*` ne produisent pas de sortie visible directe.
