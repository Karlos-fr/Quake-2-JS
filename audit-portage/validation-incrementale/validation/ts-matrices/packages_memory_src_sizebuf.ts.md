...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `packages/memory/src/sizebuf.ts` | interface | `sizebuf_t` | oui | `sizebuf_t` | `Quake-2-master/qcommon/qcommon.h` | Ported |  | Source inconnue | A verifier | Aucune entite C/H correspondante trouvee dans les matrices source. |
| `packages/memory/src/sizebuf.ts` | function | `createSizeBuffer` | oui |  |  | New |  | Entete incomplet | A verifier | Entete incomplet: `Original name` et `Source` sont requis pour le croisement. |
...
| `packages/memory/src/sizebuf.ts` | function | `encodeCString` | non |  |  | New |  | Entete incomplet | A verifier | Entete incomplet: `Original name` et `Source` sont requis pour le croisement. |
