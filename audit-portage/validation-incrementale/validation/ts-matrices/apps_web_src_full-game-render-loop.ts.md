...
| `apps/web/src/full-game-render-loop.ts` | function | `createFullGameRenderLoop` | oui | `N/A` | `N/A (web renderer adapter)` | New |  | TS sans lien source | Valide | Shared browser render orchestration; consumed by `full-game.ts`; no C/H owner expected. |
