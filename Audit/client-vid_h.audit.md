# Audit Portage Quake II - client/vid.h
...
Risque principal : faible ; le header public est porte comme contrat explicite, tandis que les implementations concretes video/menu restent derriere hooks ou dans `vid-menu.ts`.

## Source verifiee

- Source C/H : `Quake-2-master/client/vid.h`
- Port TS : `packages/client/src/vid.ts`, `packages/client/src/screen.ts`
- Consommateurs : `packages/client/src/index.ts`, modules menu client, `packages/client/src/vid-menu.ts`

## Fiche d'identification

- Fichier audite : `client/vid.h`
...
- Type de fichier : header public video
...
- Role attendu : exposer `vrect_t`, `viddef_t`, le global `viddef` et les points d'entree publics `VID_*`.
- Consommateurs directs : `packages/client/src/index.ts`, `packages/client/src/menu-misc.ts`, menu runtime
- Consommateurs finaux : menus client, backend video via hooks, controller `vid-menu.ts`
- Tests existants : `npm run verify:vid:header`, `npm run verify:menu`
...
|---|---|---|---|---|---|
| `vrect_t` | struct | `packages/client/src/screen.ts`, `packages/client/src/vid.ts` | `vrect_t` | OK | Reexporte depuis `vid.ts`. |
| `viddef_t` | struct | `packages/client/src/vid.ts` | `viddef_t` | OK | Champs preserves. |
| `extern viddef` | global | `packages/client/src/vid.ts` | `ClientVidContext.viddef` | OK avec ecart | Global remplace par contexte. |
| `VID_Init` | fonction | `packages/client/src/vid.ts` | `VID_Init` | OK | Forward hook. |
| `VID_Shutdown` | fonction | `packages/client/src/vid.ts` | `VID_Shutdown` | OK | Forward hook. |
| `VID_CheckChanges` | fonction | `packages/client/src/vid.ts` | `VID_CheckChanges` | OK | Forward hook. |
| `VID_MenuInit` | fonction | `packages/client/src/vid.ts` | `VID_MenuInit` | OK | Branche vers hooks / `vid-menu.ts`. |
| `VID_MenuDraw` | fonction | `packages/client/src/vid.ts` | `VID_MenuDraw` | OK | Branche vers hooks / `vid-menu.ts`. |
| `VID_MenuKey` | fonction | `packages/client/src/vid.ts` | `VID_MenuKey` | OK | Retour `string | null`. |
...
- [x] Si applicable, les sorties sont raccordees a `packages/renderer-common`. N/A pour ce header public.
- [x] Si applicable, les sorties sont raccordees a `packages/renderer-three`. N/A pour ce header public.
...
1. [Info] Documentation de `vid.ts` renforcee pendant l'audit.
   - Fichier/ligne : `packages/client/src/vid.ts`
   - Source originale : `client/vid.h`
...
- Reporter : non
- Documenter : reexport `vrect_t`, contexte `viddef` et forwarding `VID_*` documentes.

## Mise a jour du suivi

- Ligne `PORTAGE_QUAKE2.md` mise a jour : oui
- Nouveau statut `Valide` : ✅
- Fichier d'audit cree dans `Audit/` : `Audit/client-vid_h.audit.md`
