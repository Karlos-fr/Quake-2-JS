...
Risque principal : les bitmasks protocole `U_*` divergent du C source, puis plusieurs effets runtime de `CL_AddPacketEntities` restent incomplets.
...
- Consommateurs : a verifier

## Fiche d'identification

- Fichier audite : `client/cl_ents.c`
- Source C/H principale : `Quake-2-master/client/cl_ents.c`
- Sources C/H secondaires : `Quake-2-master/client/client.h`, protocole et q_shared equivalents
...
- Consommateurs directs : a verifier
- Consommateurs finaux : renderer three, web, audio a verifier
...
|---|---|---|---|---|---|
| `CL_ParseEntityBits` | Fonction | `packages/client/src/parse.ts` | `CL_ParseEntityBits` | Non valide | structure fidele mais `U_MOREBITS2/3` importes depuis bitmasks divergents |
...
| `CL_FireEntityEvents` | Fonction | `packages/client/src/entities.ts` | `CL_FireEntityEvents` | Non valide | `EF_TELEPORTER` extrait mais consommation particules runtime non confirmee |
...
| `CL_AddPacketEntities` | Fonction | `packages/client/src/entities.ts`, `packages/client/src/refresh.ts` | `CL_BuildPacketEntitySnapshots`, `CL_BuildRefreshFrame` | Non valide | color-shell separee et trails/particules incomplets |
...
| `CL_AddEntities` | Fonction | `packages/client/src/refresh.ts`, `packages/client/src/view.ts` | `CL_BuildRefreshFrame`, `V_RenderView` | Non valide | depend des items ouverts |
| `CL_GetEntitySoundOrigin` | Fonction | `packages/client/src/refresh.ts` | `CL_GetEntitySoundOrigin` | Verifie | |
| `frame_t`, `centity_t`, `client_state_t` | Types | `packages/client/src/types.ts` | memes noms | Partiel | `frame_t` et `centity_t` verifies, `client_state_t` reste a verifier |
| `entity_state_t`, `player_state_t` | Types | `packages/qcommon/src/q-shared.ts` | memes noms | A verifier | |
| `MAX_PARSE_ENTITIES`, `U_*`, `EF_*`, `RF_*` | Constantes | `packages/client/src/types.ts`, `packages/qcommon/src/*` | memes noms | Non valide | `MAX_PARSE_ENTITIES`, `EF_*`, `RF_*` OK a premiere vue ; `U_*` diverge du source C |
...
- [ ] `CL_ParseEntityBits`
  ...
  - Non valide en l'etat : les constantes `U_MOREBITS2/3` importees divergent du source C.
  - Ecart secondaire : `bitcounts[32]` de profiling protocole n'est pas porte ; aucun effet runtime sur l'etat client.
- [ ] `CL_ParseDelta`
...
- [ ] `CL_ParseFrame`
  ...
  - Reste ouvert : le port TS ne reproduit pas encore clairement le `SCR_EndLoadingPlaque()` conditionnel lorsque la premiere frame valide active le client.
...
- [ ] `CL_FireEntityEvents`
  ...
  - Reste ouvert : le port extrait bien les `state.event` et expose `EF_TELEPORTER` comme evenement structure, mais le chemin verifie ne declenche pas encore clairement `CL_TeleporterParticles(runtime, state)` comme le C.
- [ ] `CL_AddPacketEntities`
  ...
  - Valide partiellement : `autorotate`, `autoanim`, promotions `EF_PENT/EF_QUAD/EF_DOUBLE/EF_HALF_DAMAGE`, interpolation origine/angles, `RF_BEAM`, viewer entity skip avec lumieres, linked models `modelindex2/3/4`, powerscreen et lumieres dynamiques principales.
  - Reste ouvert / non conforme : entite color-shell separee absente du refresh, et plusieurs trails/particules automatiques du source ne sont pas executes par `CL_ExecutePacketEntityEffects`.
...
5. [Haute] `EF_TELEPORTER` extrait par `CL_FireEntityEvents` mais pas consomme comme particules runtime dans le chemin verifie
   ...
   - Impact : les particules teleporter permanentes peuvent etre absentes meme si l'evenement structure est visible dans les tests
...
8. [Moyenne] `CL_AddViewWeapon` ne reproduit pas clairement la garde `if (!gun.model) return`
...
9. [Moyenne] Header de fonction incomplet pour le port principal de `CL_AddPacketEntities`
...
10. [Critique] Les bitmasks `U_*` ne correspondent pas au source original
   - Fichier/ligne : `packages/qcommon/src/protocol.ts:79`
   - Source originale : `Quake-2-master/qcommon/qcommon.h:300`
   - Impact : le parsing delta entity n'est pas ISO protocole Quake II pour les messages originaux ; `U_EFFECTS8`, `U_MOREBITS2`, `U_SKIN8`, `U_FRAME16`, `U_RENDERFX16`, `U_EFFECTS16`, `U_MODEL2`, `U_MODEL3`, `U_MODEL4`, `U_MOREBITS3`, `U_OLDORIGIN`, `U_SKIN16`, `U_SOUND`, `U_SOLID` sont decales d'un bit par rapport au C source
   - Correction recommandee : realigner `packages/qcommon/src/protocol.ts` sur `qcommon/qcommon.h` puis verifier les encodeurs/parseurs serveur-client ensemble
