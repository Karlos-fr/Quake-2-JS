/**
 * File: quake2-entities-phase7-origin-audit.ts
 * Purpose: Verify that visible MD2 world entities keep their gameplay/runtime origins through the client refresh path without adapter-side positional offsets.
 *
 * This file is not a direct source port.
 * It is a verification harness for the origin/offset audit portion of phase 7.
 *
 * Dependencies:
 * - packages/formats
 * - packages/game
...
/**
 * Category: New
 * Purpose: Verify that rendered MD2 world entities inherit gameplay origins directly and print the encountered visible classnames.
 */
function main(): void {
  ...
  const visibleClassnames = new Set<string>();
  ...
  for (const mapPath of MAP_PATHS) {
    ...
    for (const entity of refresh.entities) {
      const gameplayEntity = runtime.entities[entity.entityNumber];
      const modelPath = modelindexToPath.get(entity.modelindex) ?? "";
      if (!gameplayEntity?.inuse || !modelPath.endsWith(".md2") || modelPath.startsWith("*")) {
        continue;
      }

      visibleClassnames.add(gameplayEntity.classname);
      assertVectorEquals(entity.origin, gameplayEntity.origin, `${mapPath} ${gameplayEntity.classname} origin`);
    }
  }

  console.log("Verification phase 7 - world MD2 origin audit OK");
  console.log(`maps: ${MAP_PATHS.join(", ")}`);
  console.log(`classnames: ${[...visibleClassnames].sort().join(", ")}`);
}

/**
 * Category: New
 * Purpose: Create one initialized gameplay runtime and advance it beyond delayed item placement.
 */
...
/**
 * Category: New
 * Purpose: Assert that two 3D vectors match exactly in the current deterministic local serialization path.
 */
