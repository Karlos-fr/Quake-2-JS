/**
 * File: quake2-m-boss32-source-parity.ts
 * Purpose: Verify `packages/game/src/m_boss32.ts` directly against `Quake-2-master/game/m_boss32.c`.
 *
 * This file is not a direct source port.
 * It is a source-parity harness that keeps the TypeScript Makron tables aligned with the original C file.
 *
 * Dependencies:
 * - Quake-2-master/game/m_boss32.c
...
import * as boss32 from "../../packages/game/src/m_boss32.js";

const SOURCE_PATH = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../Quake-2-master/game/m_boss32.c");
...
function main(): void {
  ...
  console.log("quake2-m-boss32-source-parity: ok");
}
...
function verifySourceFunctionsAreExported(): void {
  ...
  for (const functionName of sourceFunctions) {
    assert.notEqual((boss32 as Record<string, unknown>)[functionName], undefined, `${functionName} should be exported by m_boss32.ts`);
  }
}
...
function verifySourcePrecacheAssets(): void {
  const precacheBlock = getFunctionBlock("MakronPrecache", sourceWithoutComments);
  const sourceSounds = Array.from(precacheBlock.matchAll(/gi\.soundindex\s*\(\s*"([^"]+)"\s*\)/g), (match) => match[1]);
  const sourceModel = precacheBlock.match(/gi\.modelindex\s*\(\s*"([^"]+)"\s*\)/)?.[1];

  const runtime = createGameRuntimeFromBspEntities([{ properties: { classname: "worldspawn" } }]);
  boss32.MakronPrecache(runtime);

  assert.deepEqual(runtime.assets.soundPaths, sourceSounds, "MakronPrecache sound order");
  assert.equal(runtime.assets.modelPaths[0], sourceModel, "MakronPrecache rider model");

  const entity = createRuntimeEntity({ classname: "monster_makron" }, 1);
  runtime.entities[1] = entity;
  boss32.SP_monster_makron(entity, runtime);
  assert.equal(runtime.assets.modelPaths[entity.s.modelindex - 1], sourceModel, "SP_monster_makron modelindex");
}
...
function getExport<T>(name: string): T {
  const value = (boss32 as Record<string, unknown>)[name];
  assert.notEqual(value, undefined, `${name} should be exported by m_boss32.ts`);
  return value as T;
}
