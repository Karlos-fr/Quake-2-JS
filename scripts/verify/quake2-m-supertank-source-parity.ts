/**
 * File: quake2-m-supertank-source-parity.ts
 * Purpose: Verify `packages/game/src/m_supertank.ts` directly against `Quake-2-master/game/m_supertank.c`.
 *
 * This file is not a direct source port.
 * It is a source-parity harness that keeps the TypeScript monster tables aligned with the original C file.
 *
 * Dependencies:
 * - Quake-2-master/game/m_supertank.c
...
function verifySourceFunctionsAreExported(): void {
  ...
  for (const functionName of sourceFunctions) {
    assert.notEqual(
      (supertank as Record<string, unknown>)[functionName],
      undefined,
      `${functionName} should be exported by m_supertank.ts`
    );
  }
}
...
function verifySourcePrecacheAssets(): void {
  const spawnBlock = getFunctionBlock("SP_monster_supertank", sourceWithoutComments);
  ...
  const entity = createRuntimeEntity({ classname: "monster_supertank" }, 1);
  runtime.entities[1] = entity;

  supertank.SP_monster_supertank(entity, runtime);
...
function getExport<T>(name: string): T {
  const value = (supertank as Record<string, unknown>)[name];
  assert.notEqual(value, undefined, `${name} should be exported by m_supertank.ts`);
  return value as T;
}
