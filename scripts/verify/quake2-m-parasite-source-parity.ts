/**
 * File: quake2-m-parasite-source-parity.ts
 * Purpose: Verify `packages/game/src/m_parasite.ts` directly against `Quake-2-master/game/m_parasite.c`.
 *
...
function verifySourceFunctionsAreExported(): void {
  ...
  for (const functionName of sourceFunctions) {
    assert.notEqual(
      (parasite as Record<string, unknown>)[functionName],
      undefined,
      `${functionName} should be exported by m_parasite.ts`
    );
  }
}
...
function verifySourcePrecacheAssets(): void {
  const spawnBlock = getFunctionBlock("SP_monster_parasite");
  ...
  const entity = createRuntimeEntity({ classname: "monster_parasite" }, 1);
  runtime.entities[1] = entity;

  parasite.SP_monster_parasite(entity, runtime);

  assert.deepEqual(runtime.assets.soundPaths, sourceSounds, "SP_monster_parasite sound precache order");
  assert.equal(runtime.assets.modelPaths[entity.s.modelindex - 1], sourceModel, "SP_monster_parasite model precache");
}
...
function getExport<T>(name: string): T {
  const value = (parasite as Record<string, unknown>)[name];
  assert.notEqual(value, undefined, `${name} should be exported by m_parasite.ts`);
  return value as T;
}
