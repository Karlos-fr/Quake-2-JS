/**
 * File: quake2-m-flyer-source-parity.ts
 * Purpose: Verify `packages/game/src/m_flyer.ts` directly against `Quake-2-master/game/m_flyer.c`.
 *
...
function verifySourceFunctionsAreExported(): void {
  ...
  for (const functionName of sourceFunctions) {
    assert.notEqual(
      (flyer as Record<string, unknown>)[functionName],
      undefined,
      `${functionName} should be exported by m_flyer.ts`
    );
  }
}
...
function verifySourcePrecacheAssets(): void {
  const spawnBlock = getFunctionBlock("SP_monster_flyer");
  ...
  const entity = createRuntimeEntity({ classname: "monster_flyer" }, 1);
  runtime.entities[1] = entity;

  flyer.SP_monster_flyer(entity, runtime);

  assert.deepEqual(runtime.assets.soundPaths, sourceSounds, "SP_monster_flyer sound precache order");
  assert.equal(runtime.assets.modelPaths[entity.s.modelindex - 1], sourceModel, "SP_monster_flyer model precache");
}
...
function getExport<T>(name: string): T {
  const value = (flyer as Record<string, unknown>)[name];
  assert.notEqual(value, undefined, `${name} should be exported by m_flyer.ts`);
  return value as T;
}
