/**
 * File: quake2-m-chick-source-parity.ts
 * Purpose: Verify `packages/game/src/m_chick.ts` directly against `Quake-2-master/game/m_chick.c`.
 *
...
function main(): void {
  ...
  console.log("quake2-m-chick-source-parity: ok");
}
...
function verifySourceFunctionsAreExported(): void {
  ...
  for (const functionName of sourceFunctions) {
    assert.notEqual(
      (chick as Record<string, unknown>)[functionName],
      undefined,
      `${functionName} should be exported by m_chick.ts`
    );
  }
}
...
function verifySourcePrecacheAssets(): void {
  const spawnBlock = getFunctionBlock("SP_monster_chick");
  ...
  const entity = createRuntimeEntity({ classname: "monster_chick" }, 1);
  runtime.entities[1] = entity;

  chick.SP_monster_chick(entity, runtime);

  assert.deepEqual(runtime.assets.soundPaths, sourceSounds, "SP_monster_chick sound precache order");
  assert.equal(runtime.assets.modelPaths[entity.s.modelindex - 1], sourceModel, "SP_monster_chick model precache");
}
...
function getExport<T>(name: string): T {
  const value = (chick as Record<string, unknown>)[name];
  assert.notEqual(value, undefined, `${name} should be exported by m_chick.ts`);
  return value as T;
}
