/**
 * File: quake2-m-float-source-parity.ts
 * Purpose: Verify `packages/game/src/m_float.ts` directly against `Quake-2-master/game/m_float.c`.
 *
 * This file is not a direct source port.
 * It is a source-parity harness that keeps the TypeScript monster tables aligned with the original C file.
 *
 * Dependencies:
 * - Quake-2-master/game/m_float.c
...
import * as floater from "../../packages/game/src/m_float.js";
...
function main(): void {
  ...
  console.log("quake2-m-float-source-parity: ok");
}

function verifyRandomMacroConsumers(): void {
  for (const functionName of ["floater_stand", "floater_melee", "SP_monster_floater"]) {
  ...
  for (const functionName of ["floater_wham", "floater_zap", "floater_pain"]) {
    assert.ok(/\brand\s*\(/.test(getFunctionBlock(functionName)), `${functionName} source should use integer rand()`);
    const tsBlock = getTsFunctionBlock(functionName);
    assert.ok(tsBlock.includes("randomInt("), `${functionName} TS should keep integer rand() on randomInt`);
  }
}
...
function verifySourcePrecacheAssets(): void {
  const spawnBlock = getFunctionBlock("SP_monster_floater");
  ...
  const entity = createRuntimeEntity({ classname: "monster_floater" }, 1);
  runtime.entities[1] = entity;

  floater.SP_monster_floater(entity, runtime);

  assert.deepEqual(runtime.assets.soundPaths, sourceSounds, "SP_monster_floater sound precache order");
  assert.equal(runtime.assets.modelPaths[entity.s.modelindex - 1], sourceModel, "SP_monster_floater model precache");
}
...
function getExport<T>(name: string): T {
  const value = (floater as Record<string, unknown>)[name];
  assert.notEqual(value, undefined, `${name} should be exported by m_float.ts`);
  return value as T;
}
