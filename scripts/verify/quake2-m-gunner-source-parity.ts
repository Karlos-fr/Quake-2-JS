/**
 * File: quake2-m-gunner-source-parity.ts
 * Purpose: Verify `packages/game/src/m_gunner.ts` directly against `Quake-2-master/game/m_gunner.c`.
 *
 * This file is not a direct source port.
 * It is a source-parity harness that keeps the TypeScript monster tables aligned with the original C file.
 *
 * Dependencies:
 * - Quake-2-master/game/m_gunner.c
...
import * as gunner from "../../packages/game/src/m_gunner.js";
...
function main(): void {
  ...
  console.log("quake2-m-gunner-source-parity: ok");
}
...
function verifySourceFunctionsAreExported(): void {
  ...
  for (const functionName of sourceFunctions) {
    assert.notEqual(
      (gunner as Record<string, unknown>)[functionName],
      undefined,
      `${functionName} should be exported by m_gunner.ts`
    );
  }
}
...
function verifySourcePrecacheAssets(): void {
  const spawnBlock = getFunctionBlock("SP_monster_gunner");
  ...
  const entity = createRuntimeEntity({ classname: "monster_gunner" }, 1);
  runtime.entities[1] = entity;

  gunner.SP_monster_gunner(entity, runtime);

  assert.deepEqual(runtime.assets.soundPaths, sourceSounds, "SP_monster_gunner sound precache order");
  assert.equal(runtime.assets.modelPaths[entity.s.modelindex - 1], sourceModel, "SP_monster_gunner model precache");
}
...
function getExport<T>(name: string): T {
  const value = (gunner as Record<string, unknown>)[name];
  assert.notEqual(value, undefined, `${name} should be exported by m_gunner.ts`);
  return value as T;
}
