/**
 * File: quake2-m-berserk-source-parity.ts
 * Purpose: Verify `packages/game/src/m_berserk.ts` directly against `Quake-2-master/game/m_berserk.c`.
 *
 * This file is not a direct source port.
 * It is a source-parity harness that keeps the TypeScript monster tables aligned with the original C file.
 *
 * Dependencies:
 * - Quake-2-master/game/m_berserk.c
...
import * as berserk from "../../packages/game/src/m_berserk.js";
...
function main(): void {
  ...
  console.log("quake2-m-berserk-source-parity: ok");
}
...
function verifySourceFunctionsAreExported(): void {
  ...
  for (const functionName of sourceFunctions) {
    assert.notEqual(
      (berserk as Record<string, unknown>)[functionName],
      undefined,
      `${functionName} should be exported by m_berserk.ts`
    );
  }
}
...
function verifySourcePrecacheAssets(): void {
  const spawnBlock = getFunctionBlock("SP_monster_berserk");
  ...
  const entity = createRuntimeEntity({ classname: "monster_berserk" }, 1);
  runtime.entities[1] = entity;

  berserk.SP_monster_berserk(entity, runtime);

  assert.deepEqual(runtime.assets.soundPaths, sourceSounds, "SP_monster_berserk sound precache order");
  assert.equal(runtime.assets.modelPaths[entity.s.modelindex - 1], sourceModel, "SP_monster_berserk model precache");
}
...
function getExport<T>(name: string): T {
  const value = (berserk as Record<string, unknown>)[name];
  assert.notEqual(value, undefined, `${name} should be exported by m_berserk.ts`);
  return value as T;
}
